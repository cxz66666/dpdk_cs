...
uint64_t kv_aggregation_entry[entry_size];
...
print_stats(void) {
    ...
    for (portid = 0; portid < RTE_MAX_ETHPORTS; portid++) {
        ...
        printf("\nPeriod statistics ==============================="
            ...
            period_packets_rx / (1000000 * period),
...
static size_t kv_aggregation(struct rte_mbuf *m) {
    size_t *element_number_ptr = rte_pktmbuf_mtod_offset(m, size_t *, 56);
    size_t element_num = *element_number_ptr;
    kv_element *elements = rte_pktmbuf_mtod_offset(m, kv_element *, 64);
    for (size_t i = 0; i < element_num; i++) {
        kv_aggregation_entry[elements[i].key % entry_size] += elements[i].value;
    }
    return element_num;
}
...
l2fwd_main_loop(void) {
    ...
    while (!force_quit) {
        ...
        for (i = 0; i < qconf->n_tx_rx_queue; i++) {
            ...
            if (nb_rx > 0) {
                for (j = 0; j < nb_rx; j++) {
                    tmp += kv_aggregation(pkts_burst[j]);
                }
            }
            port_statistics[port_id].rx[queueid] += tmp;
