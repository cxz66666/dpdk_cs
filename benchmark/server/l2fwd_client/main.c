...
l2fwd_dpa_mac_updating(struct rte_mbuf *m, unsigned dest_portid) {
	...
	eth = rte_pktmbuf_mtod(m, struct rte_ether_hdr *);
	if (eth->src_addr.addr_bytes[4] == 1 && eth->src_addr.addr_bytes[5] == 1) {
		eth->dst_addr = eth->src_addr;
		eth->dst_addr.addr_bytes[4] = 0xf7;
		eth->dst_addr.addr_bytes[5] = 0xde;
		eth->src_addr = l2fwd_ports_eth_addr[dest_portid];
		return true;
	}
	return false;
...
test_delay(unsigned portid) {
	...
	while (!force_quit) {
		for (i = 0; i < 1 && !force_quit; i++) {
			for (j = 0; j < 1; j++) {
				pkt[j] = rte_pktmbuf_alloc(l2fwd_pktmbuf_pool);
...
l2fwd_launch_one_lcore(__rte_unused void *dummy) {
	...
	if (qconf == NULL) {
		RTE_LOG(INFO, L2FWD, "illegal qconf %u\n", lcore_id);
		return 0;
	}

	if (qconf->tx_rx_queue_list[0] < TX_RX_QUEUE) {
		l2fwd_main_loop();
	} else {
		test_delay(0);
	}
...
int main(int argc, char **argv) {
	...
	int tx_rx_queue_count = 0, delay_queue_count = 0;
	...
	RTE_ETH_FOREACH_DEV(portid) {
		...
		tx_rx_queue_count = 0;
		delay_queue_count = 0;
		while (tx_rx_queue_count < TX_RX_QUEUE || delay_queue_count < DELAY_QUEUE) {
			...
			if (tx_rx_queue_count < TX_RX_QUEUE) {
				...
				qconf->portid = portid;
				printf("[TX/RX]Lcore %u: [R/T], port %d queue from %d to %d\n", rx_lcore_id, qconf->portid, qconf->tx_rx_queue_list[0], qconf->tx_rx_queue_list[qconf->n_tx_rx_queue - 1]);
			} else if (delay_queue_count < DELAY_QUEUE) {
				for (int i = 0; delay_queue_count < DELAY_QUEUE && i < MAX_QUEUE_PER_LCORE; i++) {
					qconf->tx_rx_queue_list[i] = TX_RX_QUEUE + delay_queue_count;
					qconf->n_tx_rx_queue++;
					delay_queue_count++;
				}
				qconf->portid = portid;
				printf("[DELAY]Lcore %u: [R/T], port %d queue from %d to %d\n", rx_lcore_id, qconf->portid, qconf->tx_rx_queue_list[0], qconf->tx_rx_queue_list[qconf->n_tx_rx_queue - 1]);
			}
		}
	}
	...
	char filename[100];
	sprintf(filename, "l2fwd_client_%ld.txt", sizeof(struct Object_22) + 42);
