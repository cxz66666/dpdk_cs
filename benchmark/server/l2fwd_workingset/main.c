			...
			if (nb_rx > 0) {
				...
				for (j = 0; j < nb_rx; j++) {
					if (rte_pktmbuf_pkt_len(pkts_burst[j]) == 1024) {
						size_t *data_ptr;
						data_ptr = rte_pktmbuf_mtod(pkts_burst[j], size_t *);
						while (1) {
							size_t tmp = *data_ptr;
							if (tmp == MAGIC_NUMBER) {
								break;
				...
				if (TEST_BANDWIDTH) {
					size_t *credits = rte_pktmbuf_mtod_offset(pkts_burst[nb_rx - 1], size_t *, sizeof(struct rte_ether_hdr) + sizeof(struct rte_ipv4_hdr) + sizeof(struct rte_udp_hdr));
					*credits = nb_rx;
				}
