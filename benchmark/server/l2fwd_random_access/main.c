 ...
 */
static uint16_t nb_rxd = 64;
static uint16_t nb_txd = 64;
...
l2fwd_main_loop(void) {
	struct rte_mbuf *pkts_burst[MAX_PKT_BURST], *pkt_last;
	unsigned lcore_id;
	...
	size_t *random_access_data = (size_t *)malloc(8 * 1024 * 1024);
	size_t random_access_array_len = 1024 * 1024;
	assert(random_access_data != NULL);
	for (size_t i = 0; i < random_access_array_len; i++) {
		random_access_data[i] = i;
	}
	...
	while (!force_quit) {
		...
		for (i = 0; i < qconf->n_tx_rx_queue; i++) {
			...
			if (nb_rx > 0) {
				uint64_t dummy = 0;
				for (j = 0; j < nb_rx; j++) {
					if (rte_pktmbuf_pkt_len(pkts_burst[j]) == 1024) {
						size_t *data_ptr;
						data_ptr = rte_pktmbuf_mtod_offset(pkts_burst[j], size_t *, 256);
						while (1) {
							size_t tmp = *data_ptr;
							if (tmp == MAGIC_NUMBER) {
								break;
							}
							dummy += *(random_access_data + (tmp % random_access_array_len));
							data_ptr++;
						}
					}
				}
				...
				//TODO: CHECK
				pkt_last = pkts_burst[nb_rx - 1];
				struct rte_ether_hdr *eth = rte_pktmbuf_mtod(pkts_burst[nb_rx - 1], struct rte_ether_hdr *);;
				struct rte_ipv4_hdr *ip_hdr = rte_pktmbuf_mtod_offset(pkts_burst[nb_rx - 1], struct rte_ipv4_hdr *, sizeof(struct rte_ether_hdr));
				eth->dst_addr = eth->src_addr;
				eth->src_addr = l2fwd_ports_eth_addr[port_id];
				ip_hdr->time_to_live = 60;

				while ((sent = rte_eth_tx_burst(port_id, queueid, &pkt_last, 1)) == 0) {
				}
