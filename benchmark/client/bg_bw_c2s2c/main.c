...
sig_atomic_t signal_waiting_receive = 0;

// server的l2fwd使用15 threads
#define nb_tx_queue 6
#define nb_rx_queue 6
#define nb_delay_queue 0
// 如果设置了nb_delay_queue，请务必将CALC_LAT设为1
#define CALC_LAT 0

#define nb_max_queue (nb_rx_queue > (nb_tx_queue + nb_delay_queue) ? nb_rx_queue : (nb_tx_queue + nb_delay_queue))
	...
	for (i = 0; i < nb_pkts; i++) {
		...
		if (ipv4_hdr->time_to_live == 60) {
			...
			if (unlikely(time < 0)) {
				...
				hdr_record_value_atomic(latency_hist, time * 10);
				port_statistics[port].total_delay[qidx] += time;
				signal_waiting_receive = 0;
			}
...
test_delay(unsigned portid, struct lcore_queue_conf *qconf) {
	...
	while (!force_quit) {
		...
		signal_waiting_receive = 1;
		uint64_t prev_tsc, diff_tsc, cur_tsc, timer_tsc;
		uint64_t timer_period_x_us = 2500000;
		prev_tsc = rte_rdtsc();
		timer_tsc = 0;
		while (!force_quit && signal_waiting_receive) {
			cur_tsc = rte_rdtsc();
...
int main(int argc, char **argv) {
	...
	rte_eal_cleanup();
	char filename[100];
	sprintf(filename, "bg_bw_c2s2c_%ld.txt", sizeof(struct OBJECT_TEST) + 42);
