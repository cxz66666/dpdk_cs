...
delay_send_package(unsigned portid, struct lcore_queue_conf *qconf) {
	...
	size_t *tmp_ptr = (size_t *)(object_test.data + (1024 - 42 - 8));
	...
	while (!force_quit) {
		...
		for (i = 0; i < 1 && !force_quit; i++) {
			...
			for (j = 0; j < SEND_PKT_BURST; j++) {
				...
				// size_t *tmp_ptr = rte_pktmbuf_mtod_offset(pkt[j], size_t *, 256);
				// for (int k = 0;k < 64;k++) {
				// 	*tmp_ptr = rte_rand_max(UINT32_MAX);
				// 	tmp_ptr++;
				// }
				// *tmp_ptr = MAGIC_NUMBER;
...
int main(int argc, char **argv) {
	...
	rte_eal_cleanup();
	char filename[100];
	sprintf(filename, "delay_workingset.txt");
